...
def read_tu_data(folder, prefix):
    ...
    if edge_attr is not None:
        edge_attr = edge_attr[:, num_edge_attributes(edge_attr)-1]
...
@register_dataset("principles")
class PrinciplesDataset(TUDataset):
    def __init__(self):
        dataset = "principles"
        path = osp.join("data", dataset)
        if not osp.exists(path):
            TUDataset(path, name=dataset)
        super(PrinciplesDataset, self).__init__(path, name=dataset)
